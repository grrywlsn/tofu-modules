...
**Nameservers only (not records):** when `nameservers` is set, this module updates the domain’s linked **root** DNS zone delegation to exactly that list. It does **not** manage DNS records (`scaleway_domain_record`) or treat Scaleway as the authoritative DNS content store—use your external DNS (e.g. Bunny) for records. The provider cannot write zone NS on [`scaleway_domain_zone`](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/resources/domain_zone); this module syncs them via the DNS API / `scw`. Omit `nameservers` to leave NS unmanaged.
...
| <a name="input_nameservers"></a> [nameservers](#input\_nameservers) | Desired nameservers for the domain's linked root DNS zone only (not DNS records).<br/>Null means nameservers are unmanaged. When set, must include at least two<br/>hostnames; the root zone is updated to exactly this list.<br/>The provider cannot set NS on scaleway\_domain\_zone; this module uses the DNS API. | `list(string)` | `null` | no |

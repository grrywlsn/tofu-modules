...
The origin TLS certificate is verified by default (`verify_ssl = true`). Set `origin_http = true` for HTTP-only origins (for example an S3 website endpoint). Bunny takes TLS SNI from the origin hostname, not the public hostname: if the origin selects certificates by SNI, it must present a cert for the record value (or use a hostname-less origin listener). Bunny's `block_no_referer` default is overridden to false so direct visits are not blocked.

The hostname is registered on the pull zone as well (`create_hostname = true`, with `tls` and `force_ssl`). The `PullZone` DNS record only points the name at Bunny's edge; the edge routes by `Host`, so without the hostname the site answers 403 and serves the fallback `*.b-cdn.net` certificate. Wildcard record names are fine — Bunny accepts `*.example.com` as a hostname.

Caching on CDN records is the pair `cache_expiration_time` (default 31919000) and `smart_cache` (default true). `cache_expiration_time` is an override that ignores the origin's `Cache-Control`, and [Smart Cache](https://bunny.net/docs/cdn/smart-cache) narrows what that override applies to: only known-static extensions and MIME types are cached, and HTML, JSON and other dynamic responses always go to the origin. That is what Bunny's own DNS-accelerated pull zones do. Set `smart_cache = false` to cache everything for `cache_expiration_time`, and expect to purge the zone on each deploy if the origin serves HTML. An `OverrideCacheTime` edge rule beats Smart Cache either way, so a record with cache edge rules is unaffected by this setting.

```hcl
module "dns" {
  source = "github.com/grrywlsn/tofu-modules.git//bunnynet-dns-zone?ref=bunnynet-dns-zone-v2.6.0"
...
| [bunnynet_pullzone_hostname.cdn](https://registry.terraform.io/providers/BunnyWay/bunnynet/0.18.0/docs/resources/pullzone_hostname) | resource |
...
| <a name="output_cdn_hostname_ids"></a> [cdn\_hostname\_ids](#output\_cdn\_hostname\_ids) | Map of cdn = true record keys (a/… or cname/…) to their pull zone hostname IDs |

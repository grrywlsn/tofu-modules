...
Bunny ignores the query string when building the cache key, so `?page=1` and `?page=2` share one cached object. Add `"querystring"` to `cache_vary` for any origin that returns different content per query parameter. CDN Acceleration (`cdn = true`) pull zones are created and owned by Bunny, and the provider exposes no way to set this on them, which is a reason to prefer an explicit pull zone for a dynamic origin.
